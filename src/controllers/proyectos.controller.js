...
const getAll = async (req, res, next) => {
  try {
    const proyectos = await proyectosService.findAll();
    res.json(proyectos);
  } catch (err) {
    next(err);
  }
...
const create = async (req, res, next) => {
  try {
    const newProyecto = await proyectosService.create(req.body);
...
const update = async (req, res, next) => {
  try {
    const updated = await proyectosService.update(req.params.id, req.body);
...
const remove = async (req, res, next) => {
  try {
    const deleted = await proyectosService.remove(req.params.id);
...
const addNota = async (req, res, next) => {
  try {
    const proyecto = await proyectosService.addNota(req.params.id, req.body);
    if (!proyecto) return res.status(404).json({ error: 'Proyecto no encontrado' });
...
const updateNota = async (req, res, next) => {
  try {
    const proyecto = await proyectosService.updateNota(req.params.id, req.params.notaId, req.body);
